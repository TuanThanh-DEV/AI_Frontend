...
class TabResultDone extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getListPrescriptionInProgress(){
        var currentUser = this.props.currentUser;
            ...
            return agent.asyncRequests.get('/prescriptionEdit/findByDone?doctorId='+ currentUser.id
            ).then(function (res) {
        ...
        if(listPrescription){
            var row = listPrescription.map(item =>{
                ...
                return (<tr key={item.id}>
                            ...
                            <a style={{color:"#fff"}} href={"/editPrescription/" + item.id}><button className="btn btn-info"  >Chi Tiết</button></a>
                            </td>
                        </tr>)
            })
        }
        return  <div className="tab-pane" id="default-justified-tab14">
...
export default translate('translations')(TabResultDone);