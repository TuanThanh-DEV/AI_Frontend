...
const selector = formValueSelector("RowEditPrescriptionItem");
...
const mapDispatchToProps = dispatch => ({
    loadPrescriptionItem: (payload) =>
        ...
        dispatch({
            meta: { form: "RowEditPrescriptionItem", field: fieldName },
            payload: value,
...
class RowEditPrescriptionItem extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    handleAdd(values, dowRow) {
        const { item , indexRow, prescriptionId, patientId} = this.props;
        var _this = this;
    ...
    render() {
        ...
        this.state.listAllStock.map(item=>{
            ...
            optionDrug.push({label:item.drug.name  + " " + item.drug.hamLuongBHYT + (item.drug.ingredient ? " " + item.drug.ingredient : "") + ' (' + item.available + ' ' + item.drug.uom + ') ', value:item.drugId})
        })
