...
const selector = formValueSelector("CheckFollowOutput");
...
const mapDispatchToProps = dispatch => ({
    updateField: (fieldName, value) =>
        dispatch({
            meta: { form: "CheckFollowOutput", field: fieldName },
            payload: value,
            type: "@@redux-form/CHANGE"
        })
});

class CheckFollowOutput extends React.Component {
    constructor() {
        super();
        this.state = {
            listStock: null,
    ...
    handleSearchForm(values) {
        ...
        return agent.asyncRequests.get('/report/checkOutputReport?fromDate=' + fromDate + '&toDate=' + toDate
                ...
                <div className="page-header page-header-default">
                    <div className="breadcrumb-line">
                        <ul className="breadcrumb">
                            ...
                            <li className="active">Thẻ Xuất Kho</li>
                        </ul>
                        ...
                        <table className="table table-xxs table-bordered" id="table-to-xls" style={{ display: 'none' }}>
                            <thead>
                                <tr className="bg-teal">
                                    ...
                                    <th data-toggle="true">Tên BS</th>
                                    <th data-toggle="true">Bệnh Nhân (Tên - Tuổi - Giới Tính)</th>
                                    <th data-toggle="true">Số Lượng Bán</th>
                                    <th data-toggle="true">Số Lượng Kê Toa</th>
                                    <th data-toggle="true">Ngày Kê Toa</th>
                                </tr>
                            </thead>
                            <tbody>
                                {rows}
                            ...
                            <table className="table table-xxs table-bordered" id="table-screen">
                                <thead>
                                    <tr className="bg-teal">
                                        ...
                                        <th data-toggle="true">Số Lượng Bán</th>
                                        <th data-toggle="true">Số Lượng Kê Toa</th>
                                        <th width="10%" data-toggle="true">Ngày Kê Toa</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {rows}
    ...
    renderRowsFromDataContent(dataContent) {
        ...
        return dataContent.map(item => {
            ...
            return (
                <tr key={item.id}>
                    <td>{currentNo}</td>
                    <td width="15%">{item.drug.ingredient}</td>
                    <td>{item.drug.name}</td>
                    <td>{item.drug.hamLuong}</td>
                    <td>{item.drug.uom}</td>
                    <td>{item.drug.salePrice}</td>
                    <td>{item.docterName}</td>
                    <td>{item.inforPatient}</td>
                    <td>{item.numberOutput}</td>
                    <td>{item.numberOutputByPrescription}</td>
                    <td>{moment(item.examineTime).format(" DD/MM/YYYY ")}</td>
                </tr>);
        });
    }
}
